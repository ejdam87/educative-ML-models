...
class Node:
    def __init__( self, is_leaf: bool, attr: Attribute, i: int=-1 ) -> None:
        self.is_leaf = is_leaf
        self.ith_attr = i # which positional attribute it was in the data (order) (-1) for leaf nodes
    ...
    def predict( self, sample: pd.Series ) -> Label:
        ...
        return self.children[ sample[ self.ith_attr ] ].predict( sample )
...
def decision_tree( data: pd.DataFrame, goal: str ) -> DecisionTree:
    """
...
def decision_tree_rec( data: pd.DataFrame, attr_val: str, seen_attrs: set[ str ] ) -> Node:
    ...
    seen_attrs.add( attr )
    ith_attr = data.columns.get_loc( attr )

    new = Node( is_leaf=False, attr=attr, i=ith_attr )
