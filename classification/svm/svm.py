...
def accuracy_CM(confusion_matrix):
    return ( confusion_matrix[ 0, 0 ] + confusion_matrix[ 1, 1 ] ) / ( numpy.sum( confusion_matrix ) )
...
def kdm_RBF(X, Y, sigma):
    return kdm_B(X, Y, sigma)

def kdm_polynomial(X, Y, degrees):
    return kdm_A(X, Y, degrees)

Kernel.kfunctions['RBF'] = kdm_RBF ### Radial Basis Function
Kernel.kfunctions['polynomial'] = kdm_polynomial ### polynomial kernel
...
def get_hyperplane(multipliers, X, y):
    mult = multipliers[:, numpy.newaxis] * y[:, numpy.newaxis] * X
    return numpy.sum(mult, axis=0)
...
def prepare_svm_model(X, y, c, ktype="linear", kparams={}):
    ...
    # print(lagrange_multipliers[support_vector_indices])
    # print(numpy.where(support_vector_indices)[0])
    ...
    max_pos = numpy.max( pvs[ model["support_vector_labels"] == 1 ] )
    min_neg = numpy.min( pvs[ model["support_vector_labels"] == -1 ] )
    model["bias"] = -(max_pos + min_neg) / 2
    return model, support_vector_indices


def visu_plot_svm(train_set, test_set, model, svi=None, show_coefs=False):
    ...
    if show_coefs:
        print("Hyperplane coeficients: ", coeffs)

    # print("Hyper-plane coeffs: ", coeffs)
    sv_points = []
    ...
    axe.set_ylabel("petal length")
    plt.show()

def run_holdout_experiment(data_params, algo_params, ratio_train = .8, exercise=False):
    dataset, head, classes = load_csv(**data_params)
    run_w_data(dataset, algo_params, ratio_train, exercise)

def run_w_data(dataset, algo_params, ratio_train = .8, exercise=False):
    ...
    print("Time taken by preparation: ", tac - tic)
    ...
    # visu_plot_svm(train_set, test_set, model, svi=None, show_coefs=True)

def evaluate_set(X_train, y_train, X_test, y_test, algo_params):
    model, svi = prepare_svm_model(X_train, y_train, **algo_params)
    t = svm_predict_vs(X_test, model)
    predicted = 1*(t>0)
    evals, cm = get_CM_evals(y_test, predicted)
    return evals["accuracy"]

def k_fold_crossval(k, data, algo_params):
    ...
    for i in range(k):
        ...
        train_data = numpy.concatenate([ data[ folds[j], : ] for j in range(k) if j != i], axis=0)
        x_train, y_train = train_data[:, :-1], train_data[:, -1]
        x_test, y_test = test_data[:, :-1], test_data[:, -1]

        accuracies.append( evaluate_set(x_train, y_train, x_test, y_test, algo_params) )


    print("Mean: ", numpy.mean(accuracies))
    print("Variance: ", numpy.var(accuracies))
